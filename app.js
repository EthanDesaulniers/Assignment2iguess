...
app.get("/", ensureAuthenticated, function(request,response){
	MongoClient.connect(url, function(err,db){
		if(err) throw err;
		var dbObj = db.db("favs");
		
		dbObj.collection("favs").find().toArray(function(err,results){
			console.log("Site Served");
			db.close();
			response.render("index",{favs:results});
		});
...
app.post("/new-entry", function(request,response){
	...
	MongoClient.connect(url,function(err, db){
		...
		dbObj.collection("favs").save(request.body, function(err,result){
